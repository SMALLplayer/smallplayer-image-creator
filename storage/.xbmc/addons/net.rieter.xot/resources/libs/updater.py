...
class Updater(xbmcgui.WindowXMLDialog):
    ...
    def UpdateChannel(self, item):
        ...
        Logger.Info("Starting update for %s", item.url)
        try:
            ...
            XbmcWrapper.ShowDialog(LanguageHelper.GetLocalizedString(LanguageHelper.RestartId), LanguageHelper.GetLocalizedString(LanguageHelper.UpdateCompleteId) % (item.name))
        except:
            try:
                zipFile.close()
            except:
            ...
            XbmcWrapper.ShowDialog(LanguageHelper.GetLocalizedString(LanguageHelper.UpdateFailedId), LanguageHelper.GetLocalizedString(LanguageHelper.UpdateFailed2Id) % (item.name))
