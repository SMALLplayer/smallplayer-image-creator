...
class CacheBase:
    ...
    def _IsCachableCode(self, code):
        ...
        return (code >= 200 and code < 300) or code == 304
    ...
    def _ShouldBeCached(self, response, cachePrivate=True, inDoubtCache=True):
        ...
        * proxy-revalidate - similar to must-revalidate, except that it only
        ...
        if response.url.startswith("https:"):
            self.__Log("No 2xx response code. Not caching.")
            return False            
                    
        cacheParameters = response.cacheParameters
        if cacheParameters.has_key("no-cache") or cacheParameters.has_key("no-store"):
            self.__Log("CacheKey No-Cache, No-Store or Must-Revalidate found. Not caching")
            return False
        
        # must revalidate means that you must revalidate after the cache became
        # stale. So after the cache expired. 
        if cacheParameters.has_key("must-revalidate") or cacheParameters.has_key("proxy-revalidate"): 
            self.__Log("CacheKey Must-Revalidate or proxy-revalidate found. Caching")
            return True
        
        if cacheParameters.has_key("private"):
            if cachePrivate:
        ...
        if cacheParameters.has_key("public"):
            self.__Log("CacheKey Public found. Caching")
            return True
        
        if cacheParameters.has_key("max-age"):
    ...
    def _MustRevalidate(self, cachedResponse):
        ...
        hasMustRevalidate = cachedResponse.cacheParameters.has_key("must-revalidate") or cachedResponse.cacheParameters.has_key("proxy-revalidate")
        hasETag = cachedResponse.cacheParameters.has_key("etag")
