...
class StreamTheWorld:
    ...
    def __make_request(self, callsign):
        ''' Make a Call to StreamTheWorld API v1.4
        '''
        host = 'playerservices.streamtheworld.com'
        req = urllib2.Request(
            'http://%s/api/livestream?version=1.4&mount=%s&lang=en' %
            (host, callsign))
...
'''if __name__ == '__main__':
        ...
        __streamtheworld__  = StreamTheWorld(sys.argv[1])
        sys.exit(os.system(
            'echo %s' % __streamtheworld__.get_stream_url(sys.argv[1])))
        sys.exit(1)
'''
