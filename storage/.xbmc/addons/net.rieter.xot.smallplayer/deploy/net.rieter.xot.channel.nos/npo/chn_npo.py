...
class Channel(chn_class.Channel):
    ...
    def PreProcessFolderList(self, data):
        ...
        if "/radio/" in self.parentItem.url:
            ...
            item = mediaitem.MediaItem("3FM Live", "http://ida.omroep.nl/aapi/?stream=http://livestreams.omroep.nl/live/npo/visualradio/3fm/3fm.isml/3fm.m3u8")
            item.icon = parent.icon
            item.thumb = None
            item.thumbUrl = "http://www.3fm.nl/data/thumb/abc_media_image/113000/113453/w210.1b764.jpg"
            item.type = 'video'
            item.complete = False
            items.append(item)
    ...
    def UpdateVideoItem(self, item):
        ...
        * set self.complete = True.
        ...
        if item.url.startswith("http://ida.omroep.nl/aapi/"):
            ...
            actualStreamData = UriHandler.Open(item.url, pb=False, proxy=self.proxy)
    ...
    def __IgnoreCookieLaw(self):
        ...
        c = cookielib.Cookie(version=0, name='npo_cc', value='30', port=None, port_specified=False, domain='.www.npo.nl', domain_specified=True, domain_initial_dot=False, path='/', path_specified=True, secure=False, expires=2327431273, discard=False, comment=None, comment_url=None, rest={'HttpOnly': None})  # , rfc2109=False)
        UriHandler.Instance().cookieJar.set_cookie(c)
