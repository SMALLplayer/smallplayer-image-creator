...
class Logger:
    ...
    * critical : Log line that indicates a problem that prevent correct execution
    ...
    @staticmethod
    def CreateLogger(logFileName, logDual, applicationName, minLogLevel=10, append=False, memoryInfoProvider=None):
        ...
        if (Logger.__logger is None):
            Logger.__logger = CustomLogger(logFileName, logDual, applicationName, minLogLevel, append, memoryInfoProvider)
    ...
    def Exists():
        ...
        return not Logger.__logger is None
...
#
#        try:
#            Logger.__logger.Trace(msg, *args, **kwargs)
#        except AttributeError:
#            print Logger.__error
#            print msg % args
#            raise
#        return
#
...
class CustomLogger:
    def __init__(self, logFileName, logDual, applicationName, minLogLevel=10, append=False, memoryInfoProvider=None):
        """Intialises the Logger Instance and opens it for writing
        ...
        print "%s :: Additional logging can be found in '%s'" % (self.applicationName, self.logFileName,)
...
if __name__ == "__main__":
    ...
    print logFile
    logger = Logger.CreateLogger(logFile, False, "Logger Unittest", TRACE, False, memInfo.getFreeMem)
    Logger.Warning("Test")
    ...
    logger = Logger.CreateLogger(logFile, False, "Logger Unittest", TRACE, False)
    Logger.Warning("Test")
    Logger.Warning("Test with parameters: '%s'", "OK")
    Logger.Warning("Test with parameters: '%-5s'", 3)
    Logger.Warning("Test with parameters: '%-5s'", 3)
