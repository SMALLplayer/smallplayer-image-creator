...
class JsonHelper:
    def __init__(self, data, logger=None):
        ...
        # quotes.
        self.data = JsonHelper.ConvertSpecialChars(self.data, doQuotes=False)
        ...
        self.__pointer = 0
        self.__maxPointer = len(self.data)

    def GetNamedValues(self, name, fixQuotes=True):
        """Returns the values of the named tag with <name>

        Arguments:
        name            : string  - name of the element which value should be returned.

        Keyword Arguments:
        fixQuotes       : boolean - if True the results will be fixed by calling
                                    JsonHelper.__ConvertQuotes

        Returns:
        the value corresponding to the name. If no match is found an empty
        list is returned. The return value is already utf-8 encoded.

        """

        # the new regex also matches the arrays
        nodeRegex = '[{,]\W*?@{0,1}%s@{0,1} *: *(?:@(.*?)(?=@^@?[,}])|(\d+|false|true)(?=\W*[,}])|(\[[^]]+\])(?=\W*[,}]))' % (name,)
        nodeRegex = nodeRegex.replace('^@', "[^'\"]").replace('@', "['\"]")
        if self.Logger:
            self.Logger.Trace(nodeRegex)  # -> [{,]\W*?['"]{0,1}title1['"]{0,1} *: *(?:['"](.*?)(?=['"][^'"]*[,}])|(\d+|false|true)(?=\W*[,}]))
        results = self.__DoRegex(nodeRegex, self.data)
        # Logger.Debug(results)

        if len(results) > 0:
            # map the results to a single result
            results = map(lambda r: "%s%s%s" % (r[0], r[1], r[2]), results)

            # should we un-quote the results?
            if fixQuotes:
                results = map(lambda r: JsonHelper.__ConvertQuotes(r), results)

            return results
        else:
            return []

    def GetNamedValue(self, name, index=0):
        ...
        Keyword Arguments:
        index : int   - the index to return if multiple items are found. Defaults
                        to the first (0) index.
        ...
        # get all the results, without replacing specials chars (it would be a waste of CPU).
        result = self.GetNamedValues(name, fixQuotes=False)
        if len(result) > 0:
            return JsonHelper.__ConvertQuotes(result[index])
        else:
            return ""
    ...
    def GetJsonData(self):
        ...
        try:
            if self.data[self.__pointer] == "[":
                self.json = self.__ProcessList()
            elif self.data[self.__pointer] == "{":
                self.json = self.__ProcessDictionary()
            elif self.__Seek("("):
    ...
    def __ProcessDictionary(self):
        ...
        while char:
            if char in self.keyValueEnd:
                ...
                if key is None:
#                    if self.__pointer < 2:
#                        # it could be that the main one does not have a keyword
#                        return self.__ProcessDictionary()
#
            ...
            elif char in "tTfFnN" and not key is None:
    ...
    def __ProcessString(self, quoteChar=None):
        ...
        if escapedQuotesPresent:
            return JsonHelper.__ConvertQuotes(data)
        else:
            return data
...
if __name__ == "__main__":
    ...
    # response = opener.open("http://iphone-api.uitzendinggemist.nl/v1/episodes/")#search/+")
    ...
    lg = Logger.CreateLogger("c:\\temp\\json.log", False, "Json Unittest", 0)
    lg.Debug("test")
    ...
    print wrapper.json
    # print "Lenght: %s" % (len(str(wrapper.json)),) #221249
    ...
    for i in range(0, 5):
        # print i
        results = wrapper.GetNamedValues("description")
        results2 = wrapper.GetNamedValues("firstPlayableEpisodeVideoId")
        results3 = wrapper.GetNamedValues("title1")
